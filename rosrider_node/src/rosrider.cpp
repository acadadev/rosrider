			...
			if(crc8ccitt(status_buffer, 29) == packet_checksum) {
				...
				if(pub_diagnostics) {
		            ...
		            diag_message.bus_voltage = (status_buffer[11] + (status_buffer[10] << 8)) / 1000.0;
		       		...
		       		diag_message.cs_left = (status_buffer[13] + (status_buffer[12] << 8)) * (1.65 / 4095 / 2.0);    // 6.6 / 4096
		            diag_message.cs_right = (status_buffer[15] + (status_buffer[14] << 8)) * (1.65 / 4095 / 2.0);
